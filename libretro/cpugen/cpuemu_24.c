...
{	uaecptr dsta;
	...
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240027; }
}
}}}	m68k_incpci (4);
l_240027: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240028; }
}
}}}	m68k_incpci (6);
l_240028: ;
return;
...
{	dsta = x_get_disp_ea_040 (m68k_areg (regs, dstreg), 0);
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240029; }
}
}}}}l_240029: ;
return;
}
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240030; }
}
}}}	m68k_incpci (6);
l_240030: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240031; }
}
}}}	m68k_incpci (8);
l_240031: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240032; }
}
}}}	m68k_incpci (6);
l_240032: ;
return;
...
{	tmppc = m68k_getpci ();
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240033; }
}
}}}}l_240033: ;
return;
}
...
{	uae_u16 val  = (x_get_byte (memp) & 0xff) << 8;
	        val |= (x_get_byte (memp + 2) & 0xff);
...
void REGPARAM2 CPUFUNC(op_0148_24)(uae_u32 opcode)
{
	...
	OpcodeFamily = 29;
{	uaecptr memp = m68k_areg (regs, srcreg) + (uae_s32)(uae_s16)get_iword_cache_040 (2);
{	uae_u32 val  = (x_get_byte (memp) & 0xff) << 24;
	        val |= (x_get_byte (memp + 2) & 0xff) << 16;
	        val |= (x_get_byte (memp + 4) & 0xff) << 8;
	        val |= (x_get_byte (memp + 6) & 0xff);
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240099; }
}
}}}	m68k_incpci (4);
l_240099: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240100; }
}
}}}	m68k_incpci (6);
l_240100: ;
return;
...
{	dsta = x_get_disp_ea_040 (m68k_areg (regs, dstreg), 0);
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240101; }
}
}}}}l_240101: ;
return;
}
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240102; }
}
}}}	m68k_incpci (6);
l_240102: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240103; }
}
}}}	m68k_incpci (8);
l_240103: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240104; }
}
}}}	m68k_incpci (6);
l_240104: ;
return;
...
{	tmppc = m68k_getpci ();
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240105; }
}
}}}}l_240105: ;
return;
}
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240130; }
}
}}}	m68k_incpci (4);
l_240130: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240131; }
}
}}}	m68k_incpci (6);
l_240131: ;
return;
...
{	dsta = x_get_disp_ea_040 (m68k_areg (regs, dstreg), 0);
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240132; }
}
}}}}l_240132: ;
return;
}
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240133; }
}
}}}	m68k_incpci (6);
l_240133: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240134; }
}
}}}	m68k_incpci (8);
l_240134: ;
return;
...
{	uaecptr dsta;
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240135; }
}
}}}	m68k_incpci (6);
l_240135: ;
return;
...
{	tmppc = m68k_getpci ();
	...
	SET_ZFLG (upper == reg || lower == reg);
	SET_CFLG_ALWAYS (lower <= upper ? reg < lower || reg > upper : reg > upper || reg < lower);
	if ((extra & 0x800) && GET_CFLG ()) { Exception_cpu(6); goto l_240136; }
}
}}}}l_240136: ;
return;
}
...
{	uae_u16 newsr; uae_u32 newpc;
	for (;;) {
		...
		else { m68k_areg (regs, 7) += offset; Exception_cpu(14); goto l_240893; }
		regs.sr = newsr;
	MakeFromSR_T0();
}
	regs.sr = newsr;
...
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (0)) { Exception_cpu(7); goto l_240949; }
}}	m68k_incpci (4);
l_240949: ;
return;
}
...
{{	uae_s32 dummy;
	...
	if (cctrue (0)) { Exception_cpu(7); goto l_240950; }
}}	m68k_incpci (6);
l_240950: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (0)) { Exception_cpu(7); goto l_240951; }
}	m68k_incpci (2);
l_240951: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_51fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (1)) { Exception_cpu(7); goto l_240987; }
}}	m68k_incpci (4);
l_240987: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (1)) { Exception_cpu(7); goto l_240988; }
}}	m68k_incpci (6);
l_240988: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (1)) { Exception_cpu(7); goto l_240989; }
}	m68k_incpci (2);
l_240989: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_52fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (2)) { Exception_cpu(7); goto l_240999; }
}}	m68k_incpci (4);
l_240999: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (2)) { Exception_cpu(7); goto l_241000; }
}}	m68k_incpci (6);
l_241000: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (2)) { Exception_cpu(7); goto l_241001; }
}	m68k_incpci (2);
l_241001: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_53fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (3)) { Exception_cpu(7); goto l_241011; }
}}	m68k_incpci (4);
l_241011: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (3)) { Exception_cpu(7); goto l_241012; }
}}	m68k_incpci (6);
l_241012: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (3)) { Exception_cpu(7); goto l_241013; }
}	m68k_incpci (2);
l_241013: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_54fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (4)) { Exception_cpu(7); goto l_241023; }
}}	m68k_incpci (4);
l_241023: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (4)) { Exception_cpu(7); goto l_241024; }
}}	m68k_incpci (6);
l_241024: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (4)) { Exception_cpu(7); goto l_241025; }
}	m68k_incpci (2);
l_241025: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_55fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (5)) { Exception_cpu(7); goto l_241035; }
}}	m68k_incpci (4);
l_241035: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (5)) { Exception_cpu(7); goto l_241036; }
}}	m68k_incpci (6);
l_241036: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (5)) { Exception_cpu(7); goto l_241037; }
}	m68k_incpci (2);
l_241037: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_56fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (6)) { Exception_cpu(7); goto l_241047; }
}}	m68k_incpci (4);
l_241047: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (6)) { Exception_cpu(7); goto l_241048; }
}}	m68k_incpci (6);
l_241048: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (6)) { Exception_cpu(7); goto l_241049; }
}	m68k_incpci (2);
l_241049: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_57fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (7)) { Exception_cpu(7); goto l_241059; }
}}	m68k_incpci (4);
l_241059: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (7)) { Exception_cpu(7); goto l_241060; }
}}	m68k_incpci (6);
l_241060: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (7)) { Exception_cpu(7); goto l_241061; }
}	m68k_incpci (2);
l_241061: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_58fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (8)) { Exception_cpu(7); goto l_241071; }
}}	m68k_incpci (4);
l_241071: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (8)) { Exception_cpu(7); goto l_241072; }
}}	m68k_incpci (6);
l_241072: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (8)) { Exception_cpu(7); goto l_241073; }
}	m68k_incpci (2);
l_241073: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_59fa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (9)) { Exception_cpu(7); goto l_241083; }
}}	m68k_incpci (4);
l_241083: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (9)) { Exception_cpu(7); goto l_241084; }
}}	m68k_incpci (6);
l_241084: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (9)) { Exception_cpu(7); goto l_241085; }
}	m68k_incpci (2);
l_241085: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5afa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (10)) { Exception_cpu(7); goto l_241095; }
}}	m68k_incpci (4);
l_241095: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (10)) { Exception_cpu(7); goto l_241096; }
}}	m68k_incpci (6);
l_241096: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (10)) { Exception_cpu(7); goto l_241097; }
}	m68k_incpci (2);
l_241097: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5bfa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (11)) { Exception_cpu(7); goto l_241107; }
}}	m68k_incpci (4);
l_241107: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (11)) { Exception_cpu(7); goto l_241108; }
}}	m68k_incpci (6);
l_241108: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (11)) { Exception_cpu(7); goto l_241109; }
}	m68k_incpci (2);
l_241109: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5cfa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (12)) { Exception_cpu(7); goto l_241119; }
}}	m68k_incpci (4);
l_241119: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (12)) { Exception_cpu(7); goto l_241120; }
}}	m68k_incpci (6);
l_241120: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (12)) { Exception_cpu(7); goto l_241121; }
}	m68k_incpci (2);
l_241121: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5dfa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (13)) { Exception_cpu(7); goto l_241131; }
}}	m68k_incpci (4);
l_241131: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (13)) { Exception_cpu(7); goto l_241132; }
}}	m68k_incpci (6);
l_241132: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (13)) { Exception_cpu(7); goto l_241133; }
}	m68k_incpci (2);
l_241133: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5efa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (14)) { Exception_cpu(7); goto l_241143; }
}}	m68k_incpci (4);
l_241143: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (14)) { Exception_cpu(7); goto l_241144; }
}}	m68k_incpci (6);
l_241144: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (14)) { Exception_cpu(7); goto l_241145; }
}	m68k_incpci (2);
l_241145: ;
return;
}
...
void REGPARAM2 CPUFUNC(op_5ffa_24)(uae_u32 opcode)
{
	OpcodeFamily = 102;
{{	uae_s16 dummy = get_iword_cache_040 (2);
	if (cctrue (15)) { Exception_cpu(7); goto l_241155; }
}}	m68k_incpci (4);
l_241155: ;
return;
}
...
{{	uae_s32 dummy;
	dummy = get_ilong_cache_040 (2);
	if (cctrue (15)) { Exception_cpu(7); goto l_241156; }
}}	m68k_incpci (6);
l_241156: ;
return;
}
...
{
	OpcodeFamily = 102;
{	if (cctrue (15)) { Exception_cpu(7); goto l_241157; }
}	m68k_incpci (2);
l_241157: ;
return;
}
...
{	uae_u16 sw = x_get_iword (2);
	...
	if (!(regs.sr & 0x2000)) {
		Exception(8); goto l_241870;
	}
	regs.sr = x_get_iword (4);
	MakeFromSR();
	m68k_setstopped();
	m68k_incpci (6);
}}l_241870: ;
...
{
	OpcodeFamily = 44;
{if (!regs.s) { Exception (8); goto l_250001; }
{{	uae_s16 src = get_iword_cache_040 (2);
	uae_u16 sr = src;
	if (!(sr & 0x2000)) {
	m68k_incpci (4);
		Exception(8); goto l_250001;
	}
	regs.sr = sr;
	MakeFromSR();
	m68k_setstopped ();
...
{
	OpcodeFamily = 45;
{if (!regs.s) { Exception (8); goto l_250002; }
{	uae_u16 newsr; uae_u32 newpc;
	for (;;) {
		uaecptr a = m68k_areg (regs, 7);
		uae_u16 sr = x_get_word (a);
		...
		else { m68k_areg (regs, 7) += offset; Exception_cpu(14); goto l_250002; }
		regs.sr = newsr;
	MakeFromSR_T0();
}
	regs.sr = newsr;
